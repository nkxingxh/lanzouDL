...
{
    header('HTTP/1.1 500 Internal Server Error');
    if($OriginData == "" && MloocCurlGet("http://www.baidu.com/") == "")
    {
        if($type == 0)
        {
            die
...
function GetDownUrl($OriginData, $pwd = "", $needPwd = false, $url = "", $UserAgent = 'Mozilla/5.0 (Linux; Android 10; MAR-AL00 Build/HUAWEIMAR-AL00; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Mobile Safari/537.36')
{
    if($needPwd)
    {
        ...
        $sign = CutStr($OriginData, "'action':'downprocess','sign':'", "','p':pwd },");
        $sign = "action=downprocess&sign=" . $sign . "&p=" . $pwd;
        $OriginData = MloocCurlPost($sign, "https://" . GetHost($url) . "/ajaxm.php", $url, $UserAgent);
        $OriginData = json_decode($OriginData, true);
        if($OriginData['url'] == 0 && $OriginData['zt'] == 0)
        {
            return "PE";
        }
        else
        {
            return $OriginData['dom'] . "/file/" . $OriginData['url'];
        }
    }
    else
    {
        ...
        return $cdomain . $sts;*/
        $OriginData = CutStr($OriginData, '<script type="text/javascript">', '</script>');
        $downhref = CutStr($OriginData, "var domianload = '", "';");
        $downhref = $downhref . CutStr($OriginData, "submit.href = domianload + '", "'\n}\n\n})(document); \n");
        return $downhref;
    }  
}
...
function GetFileInfo($OriginData)
{
    ...
    $Info = substr($Info, strlen('<div class="mdo">') + 1, strpos($Info, '</div>') - strlen('<div class="mdo">') - 1);
...
{
    $str = strstr($str, $left);
    $str = substr($str, strlen($left), strpos($str, $right) - strlen($left));
    return $str;
}
